...
const RecursiveBinarySearch = (x, arr) => {
    return search(arr, x, 0, arr.length - 1);
};
function search(arr, x, leftIndex, rightIndex) {
    ...
    if (x === arr[middleIndex])
        return middleIndex;
    if (x < arr[middleIndex])
        return search(arr, x, leftIndex, middleIndex - 1);
    else
        return search(arr, x, middleIndex + 1, rightIndex);
}
